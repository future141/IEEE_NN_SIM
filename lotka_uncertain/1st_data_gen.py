...
def sys(x):
    ...
    xp[0,:] = x[0,:] + cst.del_t*cst.del_t*(-cst.a*x[1,:] + cst.b*x[3,:]*x[0,:]*x[1,:]) + x[2,:]
    xp[1,:] = x[1,:] + cst.del_t*cst.del_t*(-cst.a*x[1,:] + cst.b*x[3,:]*x[0,:]*x[1,:]) 
    return xp

# system without control
def sys_nc(x):
    xp = torch.zeros(2,1)
    xp[0,:] = x[0,:] + cst.del_t*( cst.a*x[0,:] - cst.b*x[3,:]*x[0,:]*x[1,:]) 
    xp[1,:] = x[1,:] + cst.del_t*(-cst.a*x[1,:] + cst.b*x[3,:]*x[0,:]*x[1,:])
    return xp
