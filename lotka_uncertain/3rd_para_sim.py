...
def sys_d_c(x,u):
    xp = torch.zeros(2,1)
    xp[0,:] = x[0,:] + cst.del_t*( cst.a*x[0,:] - cst.b*cst.r*x[0,:]*x[1,:]) + u
...
r_c  = torch.zeros([2,T_MAX])
x_c  = torch.zeros((2,T_MAX))
u_c  = torch.zeros((1,T_MAX-1))
u0_c = torch.zeros([1,T_MAX])
x_c[:,0] = torch.tensor([0.75,0.75]) 
cst.r = 1
r_c[:,0] = torch.tensor([0.95,0.95])
for t in range(0,T_MAX-1): 
    u0_c[0,[t]]   = 0 
    r_c[: ,[t+1]] = sys_d_c(r_c[:,[t]],u0_c[0,[t]])
# cst.B = 1.2
# simulation
cst.r = 1
for t in tqdm(range(T_MAX-1)):
    xt_c = x_c[:,[t]]
    rt_c = r_c[:,[t]]
    d_c, path_c = geodesic(rt_c,xt_c,N)
    u_c[0,t] = intgral_delta_u(rt_c,d_c,N,u0_c[0,[t]])    
    x_c[:,[t+1]] = sys_d_c(x_c[:,[t]],u_c[0,t])

t = np.linspace(0,70,T_MAX)
# plt.figure(1)
...
# plt.savefig("pics_sim/NN_C_Responds.pdf")
...
# plt.savefig("pics_sim/NN_C_R.pdf")
...
plt.plot(r[0,:],r[1,:],'.--',color='r',label = '$x^*$')
plt.plot(x[0,:],x[1,:],'.-',color='r',label = '$x$')
plt.plot(r_c[0,:],r_c[1,:],'.--',color='b',label = '$\overline{x}^*$')
plt.plot(x_c[0,:],x_c[1,:],'.-',color='b',label = '$\overline{x}$')
plt.legend()
plt.savefig("pics_sim/NN_C_U_Phase.pdf")
...
plt.legend()
plt.xlabel('$Time (day)$')
plt.savefig("pics_sim/NN_C_U_Responds_sub.pdf")
